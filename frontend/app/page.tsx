...
import { 
  ...
  Sparkles, Check, ChevronRight, Menu, X, BookOpen
} from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans flex flex-col selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* 2. Slide Drawer (Consensus Clean White Theme) */}
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 border-t border-slate-200 space-y-2">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Evidex is the AI-powered academic search engine. Search & analyze 220M+ peer-reviewed research papers.
            </p>
      ...
      <main className="flex-1 flex flex-col items-center px-4 py-12 md:py-20 max-w-4xl mx-auto w-full">
        ...
        {/* Consensus Style Search Input Box */}
        <div className="w-full max-w-2xl bg-white border border-slate-200 focus-within:border-[#0080ff] rounded-2xl p-2 transition-all shadow-sm hover:shadow-md">
          ...
          <div className="flex items-center justify-between pt-2 mt-2 border-t border-slate-100 px-2 text-xs">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200 text-slate-600 font-medium cursor-pointer hover:bg-slate-100">
                <span className="text-slate-400">+</span> Corpus
              </span>
              <span className="inline-flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200 text-slate-600 font-medium cursor-pointer hover:bg-slate-100">
                Deep <span className="text-slate-400">+</span>
              </span>
            </div>
          ...
          <div className="w-full max-w-2xl mt-8 space-y-6 text-left">
            ...
            <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-sm">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#0080ff] mb-3 flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" /> Consensus Synthesis
              </h3>
            ...
            <div className="relative rounded-2xl border border-slate-200 bg-white p-6 overflow-hidden shadow-xs">
              <div className="filter blur-sm select-none opacity-40">
                <h4 className="font-bold text-sm text-slate-800 mb-2">Pharma Funding Bias & Risk Analysis</h4>
                <p className="text-xs text-slate-500">Independent vs Industry Sponsored clinical trial audit.</p>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-xs">
                ...
                <span className="text-xs sm:text-sm font-bold text-slate-800">Unlock Full Meta-Analysis & Bias Audit</span>
            ...
            <div className="space-y-2.5">
              <h3 className="text-xs uppercase font-bold text-slate-400 tracking-wider">Referenced Studies ({data.total_studies_scanned})</h3>
              {data.studies.map((item: any) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block p-4 rounded-xl border border-slate-200 bg-white hover:border-[#0080ff] hover:shadow-xs transition-all group"
                >
                  <div className="text-sm font-semibold text-slate-800 group-hover:text-[#0080ff] transition-colors">{item.title}</div>
                  <div className="flex items-center gap-3 mt-2 text-xs text-slate-500">
                    <span>{item.source}</span>
                    <span>•</span>
                    <span>PMID: {item.pmid}</span>
                    <ExternalLink className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#0080ff] ml-auto" />
                  </div>
                </a>
              ))}
            </div>
        ...
        {/* 5. Below Search Content (Exact Consensus Sections) */}
        {!data && (
          <div className="w-full max-w-2xl mt-20 space-y-14 text-left">
            
            {/* Publisher Strip */}
            <div className="border-t border-b border-slate-100 py-8 text-center space-y-4">
              <span className="text-xs font-semibold text-slate-900">The new standard for academic research</span>
              <div className="flex flex-wrap items-center justify-center gap-8 text-slate-800 font-serif text-sm">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto pt-2">
                Search and analyze peer-reviewed literature. Evidex draws on 220M+ research papers, including full-text content.
              </p>
            </div>
              ...
              <div className="space-y-2">
                {deepSearchQueries.map((item, idx) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3.5 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-slate-100/70 hover:border-slate-300 text-xs text-slate-700 font-medium transition-all text-left group"
                  >
                    <span>{item}</span>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-slate-500">Narrow your results to top clinical guidelines and trusted journals.</p>
              <div className="space-y-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800">See where the research agrees</h3>
              </div>
              <p className="text-xs text-slate-500">Ask a clear research question and the Consensus Meter instantly shows how much evidence agrees.</p>
              <div className="p-5 rounded-2xl border border-slate-200 bg-white space-y-3.5 shadow-2xs">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs font-mono font-medium">
                    <span className="text-emerald-600 font-bold">74% Yes</span>
                    <span className="text-slate-500">18% Inconclusive</span>
                    <span className="text-rose-500">8% No</span>
                  </div>
                  <div className="w-full h-2 rounded-full bg-slate-100 flex overflow-hidden border border-slate-200">
                    ...
                    <div className="bg-slate-300 h-full" style={{ width: "18%" }} />
                    <div className="bg-rose-400 h-full" style={{ width: "8%" }} />
                  </div>
                </div>
              </div>
            </div>
